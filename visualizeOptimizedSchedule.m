function visualizeOptimizedSchedule(schedule, results, varargin)
...
%   'ShowTurnover' - Show turnover time as separate segments (default: true)
...
addParameter(p, 'Title', 'Optimized EP Lab Schedule', @ischar);
...
addParameter(p, 'ShowTurnover', true, @islogical);
...
fig = figure('Name', 'Optimized EP Lab Schedule Analysis', ...
...
title(ax1, chartTitle, 'FontSize', 16, 'FontWeight', 'bold');
xlabel(ax1, '');  % No x-label for main chart
ylabel(ax1, 'Time of Day');
...
for i = 1:numOperators
    ...
    if length(nameParts) > 1
        operatorLabels{i} = nameParts{end};
    else
        operatorLabels{i} = originalName;
    end
end
...
title(ax, 'Operator Utilization Timeline (Gray = Idle Time)', ...
