...
For the primary schedule-derived analysis, a complete operational day was defined as a calendar day for which every included case had finite, positive observed setup, procedure, and postprocedure duration values. A day containing one or more invalid required duration values was excluded in its entirety from primary laboratory-flip, operator-idle, throughput, room-gap, bottleneck-decomposition, and manuscript time-series analyses, because incomplete component timing could alter the reconstructed daily operating schedule. Case-level procedure-duration summaries retained valid observed procedure durations according to the metric-specific validity rule. As a sensitivity analysis, flip-ratio and operator idle-time results were also calculated in the sequence-valid cohort without complete-day exclusion; sensitivity outputs were not used for complete-day throughput or bottleneck estimates.

All parsed analysis options, including date range, weekday filtering, selected primary schedule cohort, manually excluded operators, minimum operator case threshold, matched and unmatched operator exclusions, and final operator-level exclusion lists, were recorded in the output data structure to support reproducibility. The software additionally recorded and reported the number of cases and days excluded for invalid component timing, the affected dates and case identifiers, and the invalid timing component for each excluded case.
